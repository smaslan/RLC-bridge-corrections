        ...
        if model.random
            ...
            for fid = 1:F
                if strcmpi(model.lin_mode,'1p')
                    ...
                    model.rng(r).gain(fid,:) = 1 + repmat(linrand(model.gain(1), model.gain(2)), [1 N]);
                ...
                elseif strcmpi(model.lin_mode,'2p')
                    ...
                    model.rng(r).gain(fid,:) = 1 + interp1([rngs(r) rngs(r+1)], [linrand(gain_a(1),gain_a(2)) linrand(model.gain(1), model.gain(2))], model.rng(r).Z, 'linear', 'extrap');
            ...
            model.rng(r).f    = freq_list;
            model.rng(r).Z    = linspace(0,1,100);
            model.rng(r).gain = repmat(1 + model.gain(1), [F 100]);
